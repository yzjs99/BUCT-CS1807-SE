<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="展览编号" prop="exhibitid">
        <el-input
          v-model="queryParams.exhibitid"
          placeholder="请输入展览编号"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:exhibitexplain:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:exhibitexplain:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:exhibitexplain:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:exhibitexplain:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="exhibitexplainList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="展览编号" align="center" prop="exhibitid" />
      ...
      <el-table-column label="讲解文字" align="center" prop="text" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="展览编号" prop="exhibitid">
          <el-input v-model="form.exhibitid" placeholder="请输入展览编号" />
        </el-form-item>
        <el-form-item label="讲解图片" prop="image">
...
<script>
import { listExhibitexplain, getExhibitexplain, delExhibitexplain, addExhibitexplain, updateExhibitexplain, exportExhibitexplain, getExhibitImage, getExhibitVoice } from "@/api/system/exhibitexplain";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        exhibitid: [
          { required: true, message: "展览编号不能为空", trigger: "blur" }
        ],
        sponsor: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listExhibitexplain(this.queryParams).then(response => {
        for (let i = 0; i < response.rows.length; i ++) {
          response.rows[i].image = getExhibitImage(response.rows[i].id);
          response.rows[i].voice = getExhibitVoice(response.rows[i].id);
        }
        this.exhibitexplainList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getExhibitexplain(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateExhibitexplain(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addExhibitexplain(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除展览讲解编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delExhibitexplain(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有展览讲解数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportExhibitexplain(queryParams);
        }).then(response => {
