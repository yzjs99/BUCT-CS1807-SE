    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:museumexplain:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:museumexplain:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:museumexplain:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:museumexplain:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="museumexplainList" @selection-change="handleSelectionChange">
...
<script>
import { listMuseumexplain, getMuseumexplain, delMuseumexplain, addMuseumexplain, updateMuseumexplain, exportMuseumexplain ,getMuseumImage,getMuseumVoice} from "@/api/system/museumexplain";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMuseumexplain(this.queryParams).then(response => {
        for (let i = 0; i < response.rows.length; i ++) {
          response.rows[i].image = getMuseumImage(response.rows[i].id);
          response.rows[i].voice = getMuseumVoice(response.rows[i].id);
        }
        this.museumexplainList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getMuseumexplain(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateMuseumexplain(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMuseumexplain(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除博物馆讲解编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delMuseumexplain(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有博物馆讲解数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportMuseumexplain(queryParams);
        }).then(response => {
